...
class Evolution():
    ...
    def derivative(self):
        ...
        Ia = np.sum(self.deltap * self.lattice.get_p_lattice()**2 * self.function * (1 + self.function)) / (2 * np.pi ** 2)
        Ib = 2 * np.sum(self.deltap * self.lattice.get_p_lattice() * self.function) / (2 * np.pi ** 2)
        T_star = Ia / Ib
        ...
        mom_deriv = (self.function[2:] - self.function[:-2]) / (self.deltap_[:-1] + self.deltap_[1:])
        ...
        Jp_ = np.interp(self.lattice.get_lattice()[1:-1],
                        self.lattice.get_p_lattice()[1:-1],
                        Jp)
        ...
        deriv = self.lattice.get_lattice()[1:]**2 * Jp_[1:] - self.lattice.get_lattice()[:-1]**2 * Jp_[:-1]
        self.Jp = Jp_
    ...
    def next_step(self):
        ...
        function_new = number_new * (6 * np.pi**2) / (self.lattice.get_lattice()[1:]**3 - self.lattice.get_lattice()[:-1]**3)
